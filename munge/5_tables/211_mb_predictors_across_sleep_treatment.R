mb_tbl_cont <- 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_ag$mb, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_probe() |> 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_sr$mb, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_probe() |> 
...
tbls[["MB_predictors_across_sleep_variables"]] <-
  mb_tbl_cont |>
  ...
  tab_header("", md("*Predictors of Mind Blanking Across Sleep Variables*"))

conditional_save(
  tbls[["MB_predictors_across_sleep_variables"]]
  , "MB - Predictors across sleep variables"
)
