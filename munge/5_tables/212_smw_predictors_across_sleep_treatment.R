smw_tbl_cont <- 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_ag$smw, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_probe() |> 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_sr$smw, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_probe() |> 
...
tbls[["SMW_predictors_across_sleep_variables"]] <- 
  smw_tbl_cont |>
  ...
  tab_header("", md("*Predictors of Spontaneous Mind Wandering Across Sleep Variables*"))

conditional_save(
  tbls[["SMW_predictors_across_sleep_variables"]]
  , "SMW - Predictors across sleep variables"
)
