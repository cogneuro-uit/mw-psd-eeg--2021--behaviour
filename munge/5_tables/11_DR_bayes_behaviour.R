...
tbls[["cont_behav_models"]] <- 
  ...
  cols_hide(c(starts_with("exc"), starts_with("c_"), starts_with("e_"))) |>
  cols_move(e, exc_bv_p) |>
  tab_style(
    cell_text(weight="bold"),
    cells_body(matches("cont_bv"),diff_bv)
  ) |>
  tab_style(
    cell_text(weight="bold"),
    cells_body(matches("cont_ae"),diff_ae)
  ) |>
  cols_hide(starts_with("diff")) |>
  tab_bayes_generics(pre_footnote = "Bold rows indicate a difference to the corresponding strict compliance Bayesian model.", 
