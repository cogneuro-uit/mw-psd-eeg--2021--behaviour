pre_neg_mood_tbl <-
   ...
   left_join(
     bayes_tbl_sum(mod.mood.dich.sc[["neg"]][["pre"]], apa_table = T, add_loo_R2 = T)  |>
     bayes_tbl_add_sig() |>
     mutate_bayes_mod_beh() |>
   ...
   left_join(
     bayes_tbl_sum(mod.mood.ext[["pre_neg~AG_PSD"]], apa_table = T, add_loo_R2 = T)  |>
       bayes_tbl_add_sig() |>
       mutate_bayes_mod_beh() |>
   ...
   left_join(
     bayes_tbl_sum(mod.mood.ext[["pre_neg~SR_PSD"]], apa_table = T, add_loo_R2 = T)  |>
       bayes_tbl_add_sig() |>
       mutate_bayes_mod_beh() |>
...
tbls[["pre_neg__pred_PSD_across_sleep_variables"]] <-
  pre_neg_mood_tbl |>
  ...
  tab_header("Predictors of Pre-Task Negative Affect")

conditional_save(
  tbls[["pre_neg__pred_PSD_across_sleep_variables"]]
  , "Pre-Negative - PSD across sleep variables"
)
