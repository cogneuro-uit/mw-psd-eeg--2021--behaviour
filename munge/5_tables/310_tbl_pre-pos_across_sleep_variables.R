pre_pos_mood_tbl <- 
   ...
   left_join(
     bayes_tbl_sum(mod.mood.ext[["pre_pos~AG_PSD"]], apa_table = T, add_loo_R2 = T)  |>
       bayes_tbl_add_sig() |>
       mutate_bayes_mod_beh() |> 
   ...
   left_join(
     bayes_tbl_sum(mod.mood.ext[["pre_pos~SR_PSD"]], apa_table = T, add_loo_R2 = T)  |>
       bayes_tbl_add_sig() |>
       mutate_bayes_mod_beh() |> 
...
tbls[["pre_pos__pred_PSD_across_sleep_variables"]] <- 
  pre_pos_mood_tbl |>
  ...
  tab_header("Predictors of Pre-Task Positive Affect")

conditional_save(
  tbls[["pre_pos__pred_PSD_across_sleep_variables"]]
  , "Pre-Positive - PSD across sleep variables"
)
