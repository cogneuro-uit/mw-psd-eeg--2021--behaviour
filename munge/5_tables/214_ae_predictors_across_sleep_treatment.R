ae_tbl_cont <- 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_ag$ae, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_beh() |> 
  ...
  left_join(
    bayes_tbl_sum(mod.cont_sr$ae, apa_table = T, add_loo_R2 = T)  |>
      bayes_tbl_add_sig() |>
      mutate_bayes_mod_beh() |> 
...
tbls[["AE_predictors_across_sleep_variables"]] <-
  ae_tbl_cont |>
  ...
  tab_header("", md("*Predictors of Approximate Entropy Across Sleep Variables*"))

conditional_save(
  tbls[["AE_predictors_across_sleep_variables"]]
  , "AE - Predictors across sleep variables"
)
