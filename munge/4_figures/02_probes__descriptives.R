# Percentage plot - visualize the effect of treatment      ======
## Data transformation      =====
plot_data <- 
  data.probe.mood.sleep |>  
  mutate(
    .by = subj
    , highlight = if_else(any(c.Adjusted_Duration.diff < -1.5), T, F),
  ) |>
  mutate(
  ...
  summarise(
    ...
    sleep = unique(c.Adjusted_Duration.diff),
...
conditional_save(
  figs[["probes__descriptives_percentage"]]
  , "Descriptives - Effect of PSD on probe responses across sample"
)

# Raw count of the probe responses for each probe quesiton     ======
##    Data transformation         ======
plot2_data <- 
  data.probe.mood.sleep |>  
  mutate(
...
plot2_data_sum <- 
  ...
  mutate(
    ...
    dataset = if_else(dataset=="exc_n", "SC", "DR"),
...
figs[["probes__descriptives_count"]] <- 
  ...
  geom_bar(data = plot2_data_sum |> filter(dataset=="SC"), 
           stat = "identity", color="black", position = position_dodge(.9), linewidth=0.2,
           width = .45) +
  scale_fill_manual(values = gen_col("/bb/rr") ) + 
  labs(x = "Response option", y = "Absolute Count", title = "b)", fill = "Sample") +
  theme(legend.position = "top", legend.direction = "horizontal")

conditional_save(
  figs[["probes__descriptives_count"]]
  , "Descriptives - Raw count for the various thought probes and their repsonse option across conditions"
)
