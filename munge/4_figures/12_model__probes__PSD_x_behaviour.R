...
figs[["MW~PSD*BV"]] <- 
  ...
  default_time_plot() +
  z_score(xlabel="Z-score BV", axis="x") +
  probe("Association with mind wandering")
...
# COMBINE       =====
figs[["MW~PSD*behav--legend"]] <- 
  patchwork::guide_area() +
  figs[["MW~PSD*BV"]]  + figs[["MW~PSD*AE"]] + labs(y="") +
...
figs[["MW~PSD*behav"]] <-
  figs[["MW~PSD*BV"]] + labs(title="b")+ figs[["MW~PSD*AE"]] + labs(y="")
...
figs[["MB~PSD*BV"]] <-
  ...
  default_time_plot() +
  z_score(xlabel="Z-score BV", axis="x") +
  probe("Associaation with mind blanking")
