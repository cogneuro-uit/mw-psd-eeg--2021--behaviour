...
subjs <- map_chr(fnames, ~str_split(.x, "/") |> 
                   pluck(1) |> pluck(-1) |>
                   str_split("_") |> 
                   pluck(1) |> pluck(1)) |> unique() 
...
actigraphy_activity <- 
  map_df(subjs, \(l_subj){
    # fnames = list.files(
    #   paste0(relative_path, "data/actigraphy"), pattern = sprintf("%s_.*.csv", l_subj), full.names = T) 
    r_fname <- fnames[str_detect(fnames, sprintf("/%s", l_subj))]
