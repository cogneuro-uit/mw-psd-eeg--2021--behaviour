...
data.probe.mood.sleep |>
  ...
  geom_point(alpha = .5) +
  geom_line(aes(col = NULL), alpha = .1) +
  ...
  scale_colour_manual(values = c(
    "#F8766D","#F8A290",  # Self report (bright red; muted red)
    "#619CFF","#8EBEFF",  # Actigraphy  (bright blue; muted blue)
    "#00BA38","#7AD686",  # Adjusted    (bright green; muted green)
    "#b20b00",            # Self report (dark red)
    "#00348C",            # Actigraphy  (dark blue)
    "#006400"             # Adjusted    (dark green) 
  )) +
  labs(x = "Average sleep deprivation", y = "Mind wandering", col = "Condition")
```

Actigraphy overestimate effects? 

## Sleep 

The estimated sleep loss during the partial sleep deprivation (PSD) compared to the normal sleep (NS) is indicated in @fig-SD-between-modalities.
...
sleeptimes_updated_trans |>
  ...
  tab_footnote(md("*Note.* Differences are calculated as self-report - actigraphy. Participant 31 has not self-reported their sleep before their normal sleep test and we therefore decided to use this participants last self-reported sleep times as a proxy for their sleep.")) |>
  ...
  tab_fmt_APA() |>
  save_output_cnd("Difference Between Self-report and Actigraphy sleep estimates")
```
...
@fig-delay-to-sleep-and-wake indicate the amount of delay from sleep attempt to sleep onset and wake time to rise time. 
```{r}
#| label: fig-delay-to-sleep-and-wake
#| include: true
#| message: false
#| warning: false
...
sleeptimes_updated |>
  ...
  summarise(
    ...
    , sleep = unique(c.Adjusted_Duration.diff.pos)
  ) |>
  pivot_wider(names_from = psd, values_from = c(sleep_quality, sleep)) |>
  mutate(sq_diff    = sleep_quality_control - sleep_quality_SD) |>
  ggplot(aes(sleep, sleep_quality, col = psd)) +
  geom_point() + 
...
<!-- #| fig-width: 10 -->
<!-- #| include: true -->
<!-- #| tbl-cap: Thought Probes Over Time Between Conditions -->
<!-- #| tbl-cap-location: top -->
...
<!--   mutate( -->
<!--     name = if_else(name == "zlogbv", "Behavioural Variability", "Approximate Entropy") |>  -->
<!--       fct_relevel("Behavioural Variability"), -->
<!--     Block    = probenum, -->
<!--     Condition = if_else(sleepdep=="SD", "PSD", "NS") |>  -->
<!--       fct_relevel("PSD", after = 0), -->
<!--   ) |> -->
<!--   ggplot(aes(Block, value, fill = Condition, col = Condition )) +  -->
<!--   facet_wrap(~name) +  -->
<!--   geom_hline(yintercept = 0, linetype = "dashed") + -->
<!--     # ALL DATA -->
<!--   stat_summary(geom = "ribbon", alpha = .25) + -->
<!--   stat_summary(geom = "line", alpha = .7) + -->
<!--   geom_smooth(method = "lm", alpha = .15, linewidth=.4) + -->
<!--   labs(y = "Centred probe response", x = "Probe number (trial)")  -->
...
```{r tabel preparation for the thought probes}
...
```{r TBL - probe models, include=TRUE, warning=FALSE, message=FALSE}
#| label: tbl-continuous-probe-models
#| tbl-cap: Bayesian Models for Mind Wandering, Mind Blanking and Spontaneous Mind Wandering
#| tbl-cap-location: top
...
probe_tbl_cont |>
  ...
  tab_bayes_generics(
    pre_footnote = "Bold rows indicate a change in the coefficient (*p*~dir~ > .95) between the continuous and reduced models.", 
    post_footnote = "BV = behavioural variability, AE = approximate entropy, Pre-positive = Pre-test positive mood, Pre-negative = Pre-test negative mood, PSD = partial sleep deprivation.") |>
  save_output_cnd("Bayesian continuous probe models +highlight")
```

<!-- OLD PROBE OVER TIME PLOT -->
<!-- ```{r FIG - probe responses over time} -->
...
<!-- save_output_cnd(p1, "Thought probes over time") -->
<!-- ``` -->

#### Behaviour


```{r TBL - ae+bv continuous models, include=TRUE, warning=FALSE, message=FALSE}
#| label: tbl-continuous-behaviour-models
#| tbl-cap: Bayesian Models for Behavioural Variability and Approximate Entropy
#| tbl-cap-location: top
...
behav_tbl |>
  ...
  tab_bayes_generics(pre_footnote = "Bold rows indicate a change in the coefficient (*p*~dir~ > .95) between the continuous and reduced sample models.", 
                     post_footnote = "Pre-positive = Pre-test positive mood, Pre-negative = Pre-test negative mood, PSD = partial sleep deprivation.") |>
  save_output_cnd("Bayesian continuous behaviour models +highlight")
```
...
```{r simple mood figure, include=TRUE, warning=FALSE, message=FALSE}
#| label: fig-simple-mood-test
#| fig-cap: Mood Change Over Test-Time and Sleep Condition
#| fig-cap-location: top
#| fig-format: svg
...
<!-- USING THE MODEL INTERACTIONS -->
```{r FIG - positive mood on BV and S-MW}
#| label: fig-behaviour-interaction-on-mw
#| fig-cap-location: top
...
p1 <- 
  interactions$smw_pos[[1]] |> 
  as_tibble() |> 
  mutate(
    type = "Pre-Task Positive Mood",
    z_score = case_when(effect2__=="6.98" ~ "-1 SD", effect2__=="13.13"~"Mean", T~"+1 SD") |>
      fct_relevel("+1 SD", "Mean")
  ) |>
  ggplot(aes(effect1__, estimate__, col = z_score)) +
  facet_wrap(~type) +
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = z_score), alpha = .15) +
  geom_line(linewidth = 1.2) +
  labs(x = "Sleep Deprivation", y = "Association to S-MW", fill = "Z-score", col = "Z-score") +
  theme(legend.position = "none", legend.direction = "horizontal")
  
p2 <- 
  interactions$bv_pos[[1]] |> 
  as_tibble() |> 
  mutate(
    type = "Pre-Task Positive Mood", 
    z_score = case_when(effect2__=="7.26" ~ "-1 SD", effect2__=="14.79"~"Mean", T~"+1 SD") |>
      fct_relevel("+1 SD", "Mean")
  ) |>
  ggplot(aes(effect1__, estimate__, col = z_score)) +
  facet_wrap(~type) + 
  geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = z_score), alpha = .15) +
  geom_line(linewidth = 1.2) +
  labs(x = "Sleep Deprivation", y = "Association to BV", fill = "Z-score", col = "Z-score") 

patchwork::guide_area() + p1 + p2 + plot_layout(guides="collect", design="11
                                                23") + theme(legend.direction = "horizontal")
```
...
expand_grid(
  ...
  mutate(
    ...
    mood  = summarised_vals$mood_m + (summarised_vals$mood_sd * sleep_deviation),
    smw_m     = mean(c$smw[,"b_pre_pos"]), 
    smw_x_psd = mean(c$smw[,"b_c.Adjusted_Duration.diff.pos:pre_pos"]),
    bv_m      = mean(c$bv[,"b_pre_pos"]),
    bv_x_psd  = mean(c$bv[,"b_c.Adjusted_Duration.diff.pos:pre_pos"]),
    bv  = bv_m* z_score + bv_x_psd * sleep * z_score,
    smw = smw_m* z_score + smw_x_psd * sleep * z_score,
    # sleep = NULL,
  ...
  add_row(
    expand_grid(
      ...
      smw = mean(c$smw[,"b_pre_pos"]) * z_score,
      bv  = mean(c$bv[,"b_pre_pos"]) * z_score
    )
  ) |>
  pivot_longer(c(bv, smw), names_to="outcome") |>
  mutate(
    outcome = if_else(outcome=="bv", "Behavioural Variability", "Spontaneous mind wandering"), 
    PSD = case_when(
  ...
  labs( y = "Association to outcome", x = "Pre-task positive mood (Z-scored)", col = "Condition", linetype="Condition") +
  # getOption("project_custom_ggplot_colours2")  #
  scale_color_manual(   values = c("NS" = "#78ADFF", "PSD -1 SD" = "#ffd6d2", "PSD Mean" = "#F8766D", "PSD +1 SD" = "#8c0000" )) +
  scale_linetype_manual(values = c("NS" = "dotted",  "PSD -1 SD" = "dashed",  "PSD Mean" = "solid",   "PSD +1 SD" = "longdash")) 
```
